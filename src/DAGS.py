...
with DAG(
    ...
    # noqa: W293 # 1. Get current datetime
    t1 = PythonOperator(
        task_id="creating_monthly_feed", python_callable=creating_monthly_feed
    )
    t2 = PythonOperator(
        task_id="Data_receiving",python_callable=Data_receiving)
    t3 = PythonOperator(
        task_id="Data_Staging",python_callable=data_staging)
    t4 = PythonOperator(
        task_id="creating_data_mart", python_callable=creating_data_mart
    )
    t1 >> t2 >> t3 >> t4"""
